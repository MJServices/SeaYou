...
class UploadResult {
  ...
  final String url;
  const UploadResult(
      {required this.bucket, required this.path, required this.url});
}
...
class UploadService {
  ...
  Future<XFile?> pickFromGallery(
      {int maxWidth = 1920, int maxHeight = 1920, int quality = 85}) async {
    try {
  ...
  Future<UploadResult?> uploadFile(
    ...
    try {
      ...
      debugPrint(
          '📁 File exists: ${await file.exists()}, size: ${await file.length()} bytes');

      await _supabase.storage.from(bucket).upload(path, file,
          fileOptions:
              FileOptions(cacheControl: cacheControl ?? '3600', upsert: false));
  ...
  static String buildPath(
      {required String userId, required String prefix, required String ext}) {
